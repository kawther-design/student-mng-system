...
<body class="text-gray-800 flex min-h-screen">
    ...
    <!-- Add Exam Modal -->
    <div id="addExamModal" class="modal-hidden fixed inset-0 z-50 flex items-center justify-center p-6 bg-school-blue/20 backdrop-blur-md transition-modal">
        <div class="bg-white w-full max-w-lg rounded-[4rem] p-12 shadow-2xl relative">
            ...
            <h3 class="text-3xl font-black text-school-blue mb-2 tracking-tighter uppercase">Schedule Exam</h3>
            ...
            <form method="POST" class="space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" required placeholder="e.g. Midterm 2026" class="w-full bg-gray-50 border-none rounded-[1.5rem] py-5 px-8 outline-none text-sm font-bold text-school-blue focus:ring-2 focus:ring-school-blue/5">
                </div>
    ...
    <aside class="fixed inset-y-0 left-0 z-40 w-72 bg-white border-r border-gray-100 hidden lg:flex flex-col p-8 shadow-2xl shadow-school-blue/5">
        <div class="flex items-center space-x-4 mb-16">
            ...
            <div>
                <h1 class="text-2xl font-black text-school-blue tracking-tighter uppercase tracking-tighter">Al Huda</h1>
    ...
    <main class="flex-1 lg:ml-72 w-full">
        <header class="bg-white/70 backdrop-blur-xl border-b border-gray-100 px-10 h-24 flex items-center justify-between sticky top-0 z-30">
            <div>
                <h2 class="text-2xl font-black text-school-blue tracking-tighter uppercase">Exam Management</h2>
        ...
        <div class="p-10">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
                <div class="glass-card p-10 rounded-[4rem]">
                    ...
                    <p class="text-4xl font-black text-school-blue tracking-tighter"><?= count($exams) ?></p>
                </div>
                <div class="glass-card p-10 rounded-[4rem]">
            ...
            <div class="bg-white rounded-[4rem] p-12 shadow-xl shadow-school-blue/5 border border-gray-50">
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="group hover:bg-school-blue/5 transition-all duration-500">
                                <td class="py-8 pl-4">
                                    <p class="font-black text-school-blue uppercase tracking-tight"><?= htmlspecialchars($exam->name) ?></p>
                                    <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1">Standard Academic Cycle</p>
                                </td>
                                <td class="py-8">
                                    <span class="px-5 py-2 bg-school-blue/5 text-school-blue rounded-full text-[9px] font-black uppercase tracking-widest"><?= htmlspecialchars($exam->type) ?></span>
                                </td>
                                <td class="py-8 text-sm text-gray-400 font-bold"><?= date('M d, Y', strtotime($exam->date)) ?></td>
                                <td class="py-8">
                                    <div class="flex items-center text-[9px] font-black uppercase tracking-widest <?= $exam->status === 'Upcoming' ? 'text-school-blue' : 'text-school-teal' ?>">
                                        <span class="w-2 h-2 rounded-full mr-3 <?= $exam->status === 'Upcoming' ? 'bg-school-blue' : 'bg-school-teal' ?> animate-pulse"></span>
                                        <?= $exam->status ?>
                                    </div>
                                </td>
                                <td class="py-8 text-right pr-4">
                                    <div class="flex items-center justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
                                        <button class="w-12 h-12 bg-school-blue/5 text-school-blue rounded-[1.2rem] flex items-center justify-center hover:bg-school-blue hover:text-white transition-all"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
